...
[![Best model R² 0.613 — 525K observations, walk-forward](https://img.shields.io/badge/Out--of--sample%20R%C2%B2-0.613%20%C2%B7%20XGBoost%20%E2%80%94%20525K%20obs,%20walk--forward-2a9d8f?style=for-the-badge)](#results)
...
<img src="outputs/figures/hero_price_volatility.png" width="900" alt="BTC price and 10-minute realized volatility, Apr 2024 - Mar 2025" />

<br />

*One year of minute-bar BTC/USDT — top: spot price (USD 60K → 108K), bottom: 10-minute realized volatility. Red line: held-out test boundary.*
...
This project builds a **production-style forecasting pipeline** end-to-end: 525,600 minute candles → 45-feature panel with strict look-ahead protection → four model families → statistically rigorous out-of-sample comparison. The best model — a Bayesian-tuned XGBoost on engineered volatility estimators — explains **61.3% of out-of-sample variance** under expanding-window walk-forward backtesting.
...
XGBoost wins on **R² and QLIKE simultaneously** — the two metrics quant practitioners actually care about. R² captures level-tracking; QLIKE (Patton 2011) is the only proper scoring rule for volatility because it penalises under-forecasting much more harshly than over-forecasting (you don't want to under-quote vol on an option you're short). The classical GARCH(1,1) baseline produces a negative R² because it systematically over-estimates vol once trained on the fat-tailed Student-t innovation distribution — a known failure mode I document explicitly rather than hide.
...
The hexbin density (log-scale) shows the prediction cloud hugs the identity line through the bulk of the distribution. The model **slightly under-shoots at the extreme right tail** (the few volatility spikes above 0.4%) — exactly where QLIKE penalty kicks in, and exactly where a production system would route the inputs to a fat-tail-aware GARCH or peaks-over-threshold adjustment.
...
Zooming into a ±18h window around the largest spike in the test set, both **XGBoost (teal)** and **the best LSTM (navy)** track the actual realized volatility (black) into a 5× spike and out of it again. The LSTM is slightly more responsive on the leading edge (sequence model picks up the build-up); XGBoost is steadier on the relaxation. This is the kind of model behaviour you would actually deploy.
...
Each stage is **idempotent**: each writes parquet/pickle that the next stage reads, so any stage can be re-run without redoing the rest. The full pipeline runs in ~3 hours on a single workstation (GARCH dominates: ~25 min × N walk-forward refits).

### Stage 1 — Data Collection & Cleaning ([`src/data_collection.py`](src/data_collection.py))

525,600 1-minute candles for BTC/USDT, Apr 2024 — Mar 2025, pulled from Binance's public archive (`data.binance.vision`, no API key). Real-world API data needs serious hardening before any ML touches it:
...
Then we build 45 features grouped by economic motivation, **every single one ending in `.shift(1)`** so a feature at minute *t* can only reference information from *t-1* or earlier. Look-ahead bias is the #1 silent killer of financial ML; the entire feature engineering module is written so that no feature can ever peek.

| Group                     | Examples                              | Captures                                  |
|---------------------------|---------------------------------------|-------------------------------------------|
| **Range vol estimators**  | Parkinson, Garman-Klass (5/10/30/60m) | High-low intraday variation, ~7× more efficient than close-to-close |
| **Rolling moments**       | std / mean / min / max / skew / kurt  | Local distributional shape across 5-60 min |
...
| **Lag features**          | log_return shifted 1-10               | Direct autoregressive memory              |
| **Cyclical time-of-day**  | sin(2πh/24), cos(2πh/24)              | Asia / EU / US session effects            |

### Stage 3-4 — Models, all under a single walk-forward harness
...
| `min_child_weight` | 7           | Suppresses noise splits — fits noisy 1-min data     |
| `reg_lambda`       | 0.68        | Strong L2 — smooths leaf weights                    |

Tuned XGBoost is then deployed in **walk-forward expanding-window mode** (refit every 500 steps on all prior data). This adds ~4 percentage points of R² over a single-fit baseline — the model picks up market regime shifts as they happen.
...
The **top three features (all rolling standard deviations of returns at 10/30/60 min) capture 63% of total importance**. Range-based volatility estimators (Parkinson, Garman-Klass) take the next tier. Everything else — lag features, OBV, taker_ratio, hour-of-day — is decoration. This is exactly what AR(1) intuition predicts: realized volatility is the dominant predictor of realized volatility.
...
Removing rolling stats or range vol estimators costs ~1.4 percentage points of R² each. Removing everything *but* lag features loses 10.8 percentage points. **The 24 rolling-statistic features alone recover 97.5% of full R²** — a meaningful pruning result for production latency. Confirmed independently by the top-K sweep:

<div align="center">
<img src="outputs/figures/xgb_topk.png" width="640" alt="Test R² vs number of features" />
</div>

5 features → 94% of full R²; 20 features → 98%. The marginal value of the 21st feature is essentially zero.

### LSTM ± Luong attention — [`VolatilityLSTM`](src/models.py)
...
- Adam (lr=1e-3, weight_decay=1e-4) with `ReduceLROnPlateau` + 10-epoch early stopping
...
- Z-score normalisation using **training-set statistics only** — vital to avoid leakage

The LSTM essentially recovers what XGBoost gets from engineered features; its only genuine win is on directional accuracy (~49% vs XGBoost's 38%) and slightly better tail tracking on the leading edge of vol spikes (see zoom plot above).

### PatchTST Transformer — [`VolatilityTransformer`](src/models.py)

PatchTST-style: input sequence chopped into patches of length 12, stride 6 → 39 patches × `d_model=128` → 2-layer encoder, 4 heads, pre-norm + GELU → mean-pool → MLP head. **Underperforms LSTM at this data scale** (R² = 0.543) — Transformers lack LSTM's sequential inductive bias (gating, forget mechanism) and need more data to discover temporal structure from scratch. A scaling experiment on 4 M minute bars narrowed the gap from 25% to 4.5% in validation loss, suggesting Transformers would catch up at >10 M samples.
...
| **QLIKE** (Patton)    | Are we under-forecasting? (Asymmetric — penalises < harder)      |
...
### Statistically defensible model comparison — Moving Block Bootstrap

Point estimates aren't enough — one test window can easily be lucky. We use the **Moving Block Bootstrap** (Künsch 1989) to test pairwise loss differentials while preserving the autocorrelation structure that destroys naive i.i.d. bootstrap on time series:
...
This produces claims of the form *"LSTM beats GARCH by Δ MSE = 4.20e-06 ± 0.31e-06, p < 0.001"* — the kind of statement that survives peer review.
...
Four-panel diagnostic for XGBoost residuals. The residual time series (top-left) is heteroskedastic — bigger errors during spikes — and the Q-Q plot (bottom-left) confirms the residuals are fat-tailed relative to Gaussian. The residual ACF (bottom-right) decays from 1.0 to <0.1 within 10 lags: the model has captured most short-horizon autocorrelation, with a small persistent signal beyond lag 20 that's the natural target for an LSTM ensemble.
...
# Stage 2 — build 45-feature panel + 10-minute RV target
python scripts/run_features.py

# Stage 3 — GARCH + XGBoost baselines (Optuna TPE search ~5 min)
python scripts/run_baselines.py            # or --garch-only / --xgb-only

# Stage 4 — Deep models (GPU recommended for LSTM grid)
...
├── docs/                         # algorithm doc + experimental findings
├── outputs/
...
- **Walk-forward backtesting** — expanding-window refit every 500 steps mirrors how a model would actually run in production, and adds ~4pp R² over a one-shot fit.
...
- **Production failure modes** — the GARCH variance-recursion bug, the LSTM gradient-explosion mitigation, the look-ahead `.shift(1)` discipline — all are documented as design choices, not afterthoughts.
