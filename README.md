...
> **The problem, intuitively.** Given the last hour of Bitcoin prices, how "jumpy" will the next 10 minutes be? Volatility isn't the *direction* of price — it's the *amplitude of wiggle*. Traders care because options prices, stop-loss sizing, and risk budgets all depend on it.
...
### 3.3 XGBoost Hyperparameter Tuning Results

After 50 Optuna TPE trials minimizing validation MSE, the best parameters found are:

| Hyperparameter | Best Value | What it means |
|----------------|-----------|---------------|
| `max_depth` | 6 | Mid-range tree depth — captures interactions without overfitting |
| `learning_rate` | 0.0682 | Moderate shrinkage per tree |
| `n_estimators` | 256 | Few trees (out of 1000 allowed) — model prefers fewer, well-regularized trees |
| `subsample` | 0.732 | ~73% row sampling — adds stochasticity to reduce variance |
| `colsample_bytree` | 0.800 | ~80% feature sampling per tree |
| `min_child_weight` | 7 | Conservative — needs 7+ samples per leaf, suppresses noisy splits |
| `gamma` | 0.000222 | Near-zero split penalty — lets informative splits through |
| `reg_alpha` (L1) | 0.151 | Light L1 — mild sparsity pressure |
| `reg_lambda` (L2) | 0.682 | Strong L2 — smooths leaf weights against overfitting |

> **Interpretation.** The tuner converged toward an **anti-overfitting** configuration: moderate depth, few trees, high min_child_weight, and strong L2 regularization. This makes sense — 1-minute financial data is extremely noisy, and most minute-to-minute variation is unpredictable. The model learns to capture the signal without memorizing the noise.

### 3.4 XGBoost Test Set Performance

| Metric | Value | Rank (out of 12 models) |
|--------|-------|------------------------|
| MSE | 1.076 × 10⁻⁷ | 5th |
| RMSE | 3.280 × 10⁻⁴ | 5th |
| MAE | 1.852 × 10⁻⁴ | 6th |
| QLIKE | **0.3210** | **1st (best)** |
| R² | **0.6131** | **1st (best)** |
| DA | 0.3772 | 12th (worst) |

XGBoost achieves the **highest R² (0.613)** and **lowest QLIKE (0.321)** across all 12 models — including every LSTM and Transformer variant. Its strength is level-tracking: it captures *how much* volatility there will be with the best calibration of any model. Its weakness is directional accuracy (37.7%): it tends to produce smooth predictions that lag rapid changes.

### 3.5 Feature Importance & Ablation

**Top 10 features by XGBoost gain** (accounting for 87% of total importance):

| Rank | Feature | Importance | Category |
|------|---------|-----------|----------|
| 1 | `ret_std_30` | 32.75% | Rolling std of returns (30-min) |
| 2 | `ret_std_10` | 17.07% | Rolling std of returns (10-min) |
| 3 | `ret_std_60` | 13.62% | Rolling std of returns (60-min) |
| 4 | `parkinson_30` | 7.05% | Parkinson volatility (30-min) |
| 5 | `garman_klass_10` | 5.42% | Garman-Klass volatility (10-min) |
| 6 | `parkinson_5` | 3.69% | Parkinson volatility (5-min) |
| 7 | `parkinson_10` | 3.19% | Parkinson volatility (10-min) |
| 8 | `bollinger_width` | 2.05% | Bollinger band width |
| 9 | `parkinson_60` | 1.33% | Parkinson volatility (60-min) |
| 10 | `garman_klass_5` | 1.28% | Garman-Klass volatility (5-min) |

> **Key insight.** Volatility features dominate overwhelmingly. The top 3 features — all rolling standard deviations — account for **63% of total gain**. This confirms the core intuition: *recent realized volatility is the strongest predictor of near-future volatility* (volatility clustering). Meanwhile, `lag_3` through `lag_10` have **zero importance** — raw lagged returns are redundant when rolling statistics are already present.

**Feature group ablation** (remove an entire group, retrain, measure R² change):

| Experiment | Features Removed | R² | Impact |
|------------|-----------------|-----|--------|
| All features (baseline) | 0 | 0.5698 | — |
| No ATR/Bollinger | 2 | 0.5700 | +0.04% (negligible) |
| No lag features | 10 | 0.5693 | -0.09% |
| No volume features | 5 | 0.5689 | -0.16% |
| No time encoding | 2 | 0.5673 | -0.44% |
| No volatility estimators | 8 | 0.5572 | **-2.21%** |
| No rolling stats | 24 | 0.5565 | **-2.33%** |
| Only lag features | — | 0.4622 | **-18.88%** |

Rolling stats and volatility estimators are the critical feature groups. Lags, volume, and time encoding are nearly redundant with them present.

**Top-K analysis** (train with only the K most important features):

| Top-K | R² | % of full model |
|-------|-----|-----------------|
| 5 | 0.5362 | 94.1% |
| 10 | 0.5477 | 96.1% |
| 20 | 0.5602 | 98.3% |
...
Just 5 features get **94% of the full model's R²**. Performance saturates around 20–30 features; the remaining 22–32 contribute only 0.15% additional R².
